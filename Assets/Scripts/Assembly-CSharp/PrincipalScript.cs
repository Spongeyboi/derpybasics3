...
public class PrincipalScript : MonoBehaviour
{
	...
	{
		if (!this.angry & !nighttime) // If the principal isn't angry
		{
		...
		else if (!this.angry & nighttime) // If the principal isn't angry
		{
			Vector3 direction = this.player.position - base.transform.position;
			RaycastHit raycastHit; // If he sees the player and the player has guilt
			if (Physics.Raycast(base.transform.position, direction, out raycastHit, float.PositiveInfinity, 3, QueryTriggerInteraction.Ignore) & raycastHit.transform.tag == "Player" & !this.inOffice & !this.angry)
			{
				this.seesRuleBreak = true;
			}
			else
	...
	private void CorrectPlayer()
	{
        ...
        if (oneroom)
        {
            this.audioQueue.QueueAudio(this.audAfterHours);
        }
		else if (this.playerScript.guiltType == "faculty")
		{
	...
	private void OnTriggerStay(Collider other)
	{
		...
		if (other.tag == "Player" & this.angry & !this.inOffice)
		{
            ...
            if (oneroom)
            {
                this.agent.Warp(new Vector3(10f, 0f, -220f)); //Teleport the principal to the office
                this.agent.isStopped = true; //Stop the principal from moving
                other.transform.position = new Vector3(10f, 4f, -220f); // Teleport the player to the office
            }
            else
            {
    ...
    public AudioClip audAfterHours;

...
public class SwingingDoorScript : MonoBehaviour
{
	...
	private void Update()
	{
		if (!this.requirementMet & this.gc.notebooks >= 0) // If the player has collected 2 notebooks, unlock the doors
		{
	...
	{
		if (!this.bDoorLocked) // If it isn't locked, make baldi hear it. And open it(but that is handled in a different piece of code for some reason)
		{
